...
export default function CategoryNewForm({ open, onClose, onCreate, onUpdate, currentCategory }) {
  ...
  const NewCategorySchema = Yup.object().shape({
    counterparty_name: Yup.string().required('Поле обязательное'),
  });

  const defaultValues = useMemo(
    () => ({
      counterparty_name: currentCategory?.counterparty_name || '',
    }),
    [currentCategory]
  );
    ...
    try {
      if (!currentCategory) {
        ...
        onUpdate(
          {
            ...data,
            counterparty_id: currentCategory?.counterparty_id,
          },
          () => {
        ...
        <DialogContent dividers>
          <Stack spacing={3}>
            <Box
              ...
              pt={2}
            >
              <RHFTextField name="counterparty_name" label="Название категории" />
            </Box>
          </Stack>
        </DialogContent>
