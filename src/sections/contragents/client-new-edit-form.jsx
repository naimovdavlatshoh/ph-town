...
export default function ClientNewEditForm({ currentClient }) {
  ...
  const NewBusinessClientSchema = Yup.object().shape({
    ...
    business_inn: Yup.string().required('Заполните поле'),
  ...
  const defaultValuesForPersonalClient = useMemo(
    () => ({
      ...
      expire_date: currentClient?.date_of_birth
        ? moment(currentClient?.date_of_birth).toDate()
        : '',
      date_of_issue: currentClient?.date_of_issue
  ...
  return (
    <FormProvider
      methods={typeEntity === 'individual' ? methodsForPersonalClient : methodsForBusinessClient}
      onSubmit={typeEntity === 'individual' ? onSubmitPersonal : onSubmitBusiness}
    >
