...
import InvoiceTableFiltersResult from '../../payments/invoice-table-filters-result';
...
const TABLE_HEAD = [
  ...
  { id: 'comment', label: 'Коммент', align: 'center' },
  { id: 'payment_amount', label: 'Сумма (UZS)', align: 'right' },
  { id: 'real_payment_amount_uzs', label: 'В кассу (UZS)', align: 'right' },
  { id: 'payment_amount_usd', label: 'Сумма (USD)', align: 'right' },
  { id: 'contract_exchange_rate', label: 'Курс контракта', align: 'right' },
  { id: 'payment_exchange_rate', label: 'Курс оплаты', align: 'right' },
...
export default function BarterContractListView() {
  ...
  const { cashInformation, bartercontracts, count, create, create2, remove } =
    useGetBarterContracts(page + 1, filters.startDate, filters.endDate, filters.client.id);

  useEffect(() => {
    setTableData(bartercontracts);
  }, [bartercontracts]);
  ...
  const notFound = (!bartercontracts.length && canReset) || !bartercontracts.length;
        ...
        <Card>
          ...
          {canReset && (
            <InvoiceTableFiltersResult
              ...
              //
              results={bartercontracts.length}
              sx={{ p: 2.5, pt: 0 }}
            />
          )}
          ...
          <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
            ...
            <Scrollbar>
              <Table size="small" sx={{ minWidth: 800 }} aria-label="a dense table">
                ...
                <TableBody>
                  {bartercontracts.map((row) => (
                    <BarterContractTableRow
                      key={row.kassa_id}
                      row={row}
                      onDeleteRow={(data, cb) => {
                        remove(data, cb);
