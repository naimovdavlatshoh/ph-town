...
import { useGetKassaBankCategories } from 'src/api/payments';
...
import Categories from './categories-table-row';
...
const TABLE_HEAD = [
  ...
  { id: 'category_name', label: 'Название' },
  { id: 'options', label: 'Редактировать', width: 50, align: 'center' },
];

export default function KassaBankCategoriesFullscreen({ open, handleClose }) {
  ...
  const {
    ...
    kassBankCategories,
    kassBankCategoriesEmpty,
    count,
    create,
    update,
  } = useGetKassaBankCategories(page);

  const [visibleAppBar, setVisibleAppBar] = useState(true);
  const [marginTop, setMarginTop] = useState(50);

  const notFound = kassBankCategoriesEmpty;
        ...
        <Scrollbar>
          <Table size="small" sx={{ minWidth: 360, mt: 10 }} stickyHeader aria-label="sticky table">
            <TableHeadCustom
              ...
              rowCount={kassBankCategories?.length}
              numSelected={table.selected.length}
              onSort={table.onSort}
            />

            <TableBody>
              {kassBankCategories?.map((row) => (
                <Categories
                  key={row.id}
                  row={row}
                  onSelectRow={() => {}}
...
KassaBankCategoriesFullscreen.propTypes = {
