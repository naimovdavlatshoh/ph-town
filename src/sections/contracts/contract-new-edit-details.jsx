...
export default function ContractNewEditDetails() {
  ...
  const renderTotal = (
    ...
    >
      <Stack direction="row">
        <Box sx={{ color: 'text.secondary' }}>Выплачено 2</Box>
  ...
  const isEqual =
    values?.mounthPayList?.reduce((prev, curr) => prev + +curr.price, 0) ===
    values.totalAmount - values.initialPayment?.replace(/,/g, '');

  const deff =
    values.totalAmount -
    values.initialPayment?.replace(/,/g, '') -
    values?.mounthPayList?.reduce((prev, curr) => prev + +curr.price, 0);
          ...
          <TableBody>
            ...
            {values.mounthPayList.map((row, index) => (
              <TableRow key={index}>
                ...
                <TableCell>{row.price ? fCurrency(row.price) : 0} сум</TableCell>
                <TableCell>
              ...
              <TableCell colSpan={4}>
                {!isEqual ? (
                  <Alert icon={<Error fontSize="inherit" />} severity="error">
                    Осталось еще <b>{fCurrency(deff)}</b> сум
                  </Alert>
                ) : (
  ...
  const PaymentMonthSchema = Yup.object().shape({
    ...
    price: Yup.string()
      ...
      .test(
        'max',
        `Максимальная допустимая сумма ${fCurrency(deff)} UZS`,
        (value) => value?.replace(/,/g, '') <= deff
      )
      .test('min', `Минимальная допустимая сумма 0 UZS`, (value) => value?.replace(/,/g, '') >= 0),
  });
  ...
  useEffect(() => {
    if (values.months && values.months > 0) {
      ...
      setValue(
        'monthly_fee',
        values.mounthPayList?.map((mp) => mp.price)
      );
    }
        ...
        <Stack divider={<Divider flexItem sx={{ borderStyle: 'dashed' }} />} spacing={3}>
          <Stack direction="row" alignItems="flex-start" spacing={2} sx={{ width: 1 }}>
            ...
            <RHFCurrencyField
              ...
              helperText={`Доступная сумма: ${fCurrency(deff)} UZS`}
            />
