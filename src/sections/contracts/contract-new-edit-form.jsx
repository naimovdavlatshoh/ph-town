  ...
  const generateDocument = async (data) => {
    ...
    const templateData = {
      ...
      contract_date: formatRussianDate(
        dayjs(data.contract_date).locale('ru').format('«D» MMMM YYYY [йил]')
      ),
      contract_date2: formatRussianDate(
        dayjs(data.contract_date).locale('ru').format('«D» MMMM YYYY [й]')
      ),
      client_name:
      ...
      total_price_text: convertNumberToUzText(data?.totalAmount),
...
function formatRussianDate(dateString) {
  ...
  const formattedMonth = months[month.toLowerCase()] || month.toLowerCase().replace(/ь$/);

  return `${day} ${formattedMonth} ${year} йил`;
}
