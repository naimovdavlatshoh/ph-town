...
export function useGetBarterContracts(page = 1, startdate = '', enddate = '', clientId) {
  const URL = endpoints.bartercontract.list;
  const DELETE_URL = endpoints.bartercontract.delete;
  const CREATE_URL = endpoints.bartercontract.pay;
  const CREATE_URL2 = endpoints.bartercontract.pay2;
  ...
  const memoizedValue = useMemo(
    () => ({
      bartercontracts: data?.option?.length ? data?.option : [],
      count: data?.count || 0,
      cashInformation: data?.cash_information?.length ? data?.cash_information : [],
      bartercontractsLoading: isLoading,
      bartercontractsError: error,
      bartercontractsValidating: isValidating,
      bartercontractsEmpty: !isLoading && !data?.option?.length,
      create,
      create2,
      remove,
    }),
...
export function useGetKassaSklad(page = 1, startdate = '', enddate = '', clientId) {
  const URL = endpoints.bartercontract.payKassaSkladList;
  const DELETE_URL = endpoints.bartercontract.deleteKassaSklad;
  const CREATE_URL = endpoints.bartercontract.pay;
  const CREATE_URL2 = endpoints.bartercontract.pay2;
  const PAY_URL = endpoints.bartercontract.payKassaSklad;
...
export function useGetKassaBankArrival(page = 1, startdate = '', enddate = '', categoryId) {
  const URL = endpoints.bartercontract.kassaBankArrivalList;
  const DELETE_URL = endpoints.bartercontract.kassaBankDeleteArrival;
  const PAY_URL = endpoints.bartercontract.kassaBankPayArrival;
...
export function useGetKassaBankCategories(page = 1) {
  const URL = endpoints.bartercontract.kassaBankCategories;
  const CREATE_URL = endpoints.bartercontract.kassaBankCategoriesCreate;
  const UPDATE_URL = endpoints.bartercontract.kassaBankCategoriesUpdate;
