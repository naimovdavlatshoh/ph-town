...
### Роли
- `user?.role` — идентификатор роли: `1`/`2` (админы/менеджеры), `3`, `5` и т.д.
  Доступ к действиям гейтится проверками вида `['1','2'].includes(user?.role)` и
  `RoleBasedGuard roles={[...]}` — списки ролей везде **строки**.
- **ВНИМАНИЕ:** `login.php` отдаёт `role` **числом** (`"role": 1`), а `auth-provider`
  кладёт ответ в стейт как есть, не нормализуя тип. Поэтому любую проверку роли надо
  кастовать к строке (`` `${user?.role}` ``), иначе `['1',...].includes(1)` === `false`.
  nav-section это уже делает; `RoleBasedGuard` чинили тем же кастом (см. журнал 2026-07-12).
...
### 2026-08-29 — Новый модуль «Парковка» (CRUD + Шахматка-Парковка)
- **Что:** добавлен функционал парковочных мест по объекту — иерархия
  **Блок парковки → Этаж → Место** (аналог квартирной шахматки, но самостоятельный API).
- **API-контракт (иное, чем у квартир):** отдельного tree-эндпоинта нет, есть только CRUD
  трёх уровней (`parkingblocklist` / `parkingfloorlist` / `parkingspotlist` + create/update/
  delete). Все списки отдают **сырой массив** (пустой = `[]`), удаление мягкое, удалить блок/
  этаж нельзя, пока внутри есть дети (400). Цена места хранится в USD (`price_usd`), а
  `price_uzs` — пересчёт по общему курсу (может быть `null`). `stock_status`: 1 свободно /
  2 продано. Эндпоинты — `endpoints.parking.*` в [axios.js](../src/utils/axios.js).
- **Данные:** [src/api/parking.js](../src/api/parking.js) — хуки `useGetParkingBlocks(projectId)`,
  `useGetParkingFloors(blockId)`, `useGetParkingSpots(floorId)` (list + create/update/remove,
  мутации через простой `mutate()`-ревалидейт) и `useGetParkingBoard(projectId)`, который
  **сам собирает дерево** (блок→этаж→место) тремя каскадными запросами для шахматки.
- **Шахматка-Парковка (показ):** `src/sections/parking/parking-board-view.jsx` (тулбар: фильтр
  статуса свободно/продано + Очистить), `parking-grid.jsx` (блоки в горизонтальном скролле,
  этажи сверху вниз, подвал = отрицательный номер → «П{n}»), `parking-spot.jsx` (ячейка,
  цвет по статусу, тултип: номер + цена USD/сум), `parking-fullscreen-dialog.jsx`.
- **CRUD (управление):** самостоятельный **диалог** `parking-manage-dialog.jsx` (аккордеон
  блоков → `parking-floors-panel.jsx` → `parking-spots-panel.jsx`), формы
  `parking-block/floor/spot-new-edit-form.jsx`. Сделан диалогом, а НЕ новыми роутами/страницами,
  чтобы не трогать paths/nav/routes — изменения хирургические.
- **Точки входа:** на карточке объекта ([object-item.jsx](../src/sections/object/object-item.jsx))
  кнопка «Шахматка-Парковка» рядом с «Шахматка» → полноэкранная шахматка; на странице
  «Наполнить» ([object-details-view.jsx](../src/sections/object/object-details-view.jsx)) кнопка
  «Парковка» (только роли 1/2) → диалог управления.
- **Не проверено вживую:** прогонял только ESLint (0 ошибок). Реального ответа бэка по
  парковке не видел — типы полей (`stock_status`, `floor_number`) приводятся через `Number()`
  на всякий случай; если сервер вернёт иные имена полей — поправить в `parking.js`/компонентах.

### 2026-07-16 — Фикс: курс доллара менял общую сумму контракта
- **Баг:** при создании контракта ввод курса доллара пересчитывал «Общую сумму»/«Итого».
  Квартира за 72 464 $ при курсе компании 13 000 стоит 942 032 000 сум, но стоило вбить
  12 000 — итог становился 869 568 000.
- **Причина:** в `1d28bc0` в `contract-new-edit-form.jsx` появились хелпер `calcTotalPrice`
  и эффект, пересчитывавший `totalAmount` на каждое нажатие в поле курса. Зеркальная
  формула одновременно появилась на бэке (`d9904d9`). Поле «Курс доллара» — **справочное**:
  оно нужно, чтобы указать курс на дату старого договора при оформлении задним числом,
  и на сумму влиять не должно.
- **Фикс (только фронт):** удалены `calcTotalPrice` и эффект пересчёта; `total_price` в
  payload снова берётся из `totalAmount`. Единственный источник суммы — `uzs_full_price`
  из `contract-new-edit-client.jsx:59`. Поле курса осталось на месте и уходит в
  `contract_exchange_rate` как данные.
- **⚠️ БАГ ЗАКРЫТ НЕ ПОЛНОСТЬЮ.** `ContractController::fillContractData()` в `api-ph-town`
  игнорирует присланный `total_price` и пересчитывает сумму сам по формуле
  `price_square_meter * area * contract_exchange_rate` (появилось в бэковом коммите
  `d9904d9`). Поэтому на экране сумма от курса больше не зависит, но **в БД по-прежнему
  ляжет сумма по введённому курсу**. Правка бэка была сделана и по решению заказчика
  откачена — трогать `api-ph-town` без отдельного разрешения нельзя.
  Чтобы закрыть до конца: в `calcTotalPrice()` брать курс компании из `tbexchangerate`
  (как в `uzs_full_price`), а не присланный `contract_exchange_rate`.
- **Следствие для рассрочки:** фронт строит график от `uzs_full_price`, а бэк проверяет его
  против своего `last_total_price` (`validateMonthlySum`). При курсе, отличном от курса
  компании, создание рассрочки упадёт с «Сумма ежемесячных платежей не совпадает с
  остатком по контракту!».
- **Важно:** сумма квартиры в сумах нигде не хранится. В БД лежит цена в долларах
  (`price_square_meter`, `totalprice`), а `uzs_full_price` — это
  `dollar_rate × площадь × цена_м2` (`api/objects/apartment.php:629`), где `dollar_rate` —
  курс компании из `tbexchangerate` (надпись в шапке, `/api/v1/dollar/rate`).
  Жёлтый виджет «Курсы валют ЦБ» в сайдбаре — это `<iframe>` с cbu.uz
  (`nav-vertical.jsx:62`), с системой он не связан и ни на что не влияет.
- **Не чинилось (пред-существующее):** в режиме редактирования смена квартиры не обновляет
  `totalAmount` — эффект в `contract-new-edit-client.jsx:54` выходит при `mode === 'edit'`.
  В БД сумма ляжет верная (бэк считает по `apartment_id`), но на экране будет старая.

### 2026-07-12 — Фикс: «Доступ запрещен» после логина (роль-число)
- **Баг:** `login.php` стал отдавать `role` числом (`"role": 1`). `RoleBasedGuard`
  сравнивал сырое число со списком строк (`['1','4','6'].includes(1)` === `false`) и
  блокировал любую страницу sidebar — при этом запрос в backend не уходил (гвард
  короткозамыкает рендер до монтирования страницы/SWR). Пункты меню были видны, т.к.
  nav-section кастует роль к строке, а гвард — нет.
- **Фикс:** в `src/auth/guard/role-based-guard.jsx` каст роли к строке —
  `!roles.includes(`${currentRole}`)` (та же конвенция, что в nav-section). Одна строка.
